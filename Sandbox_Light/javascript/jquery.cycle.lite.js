...
jQuery.fn.cycle = function(options) {
    return this.each(function() {
        ...
        jQuery.fn.cycle.transitions.fade(jQuerycont, jQueryslides, opts);
        ...
        jQueryslides.not(':eq('+first+')').css({opacity:0});
        if (opts.cssFirst)
...
jQuery.fn.cycle.custom = function(curr, next, opts, cb) {
    ...
    jQueryn.css({opacity:0});
    var fn = function() {jQueryn.animate({opacity:1}, opts.speedIn, opts.easeIn, cb)};
    jQueryl.animate({opacity:0}, opts.speedOut, opts.easeOut, function() {
        jQueryl.css({display:'none'});
        if (!opts.sync) fn();
    });
    if (opts.sync) fn();
};
